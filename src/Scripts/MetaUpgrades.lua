...
modutil.mod.Path.Wrap("BiomeSpeedTimerLoop", function(base)
	...
	if not config.disableAllTimersOutOfCombat and game.CurrentRun.ModVowOfPausedTimeTimerPaused then
	...
	if game.CurrentRun.BiomeTime <= 0 and (config.disableVowTimerOutOfCombat or config.disableAllTimersOutOfCombat) and game.BiomeTimeLimits.Penalty.Interval == game.BiomeTimeLimits.Penalty.ModVowOfPausedTimePausedTimerInterval then
